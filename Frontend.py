...
updater.addon = 'blender_rich_presence'
Extension = 'Blender RPC'
...
if 'backup_date' in updater.json and os.path.isdir(BackupPath):
  if updater.json['backup_date'] == '':
    ...
    LatestDate = ' ' + updater.json['backup_date']
BackupText = 'Restore a previous backup for Blender RPC{}'.format(LatestDate)
...
class addon_updater_search(bpy.types.Operator):
	bl_label = f'Search'
	bl_idname = updater.addon + '.updater_search'
	bl_description = f'Search for the latest version of {Extension}'
	bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_refresh(bpy.types.Operator):
	bl_label = f'Refresh'
	bl_idname = updater.addon + '.updater_refresh'
	bl_description = f'Refresh the latest version of {Extension}'
	bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_loading(bpy.types.Operator):
	bl_label = f'Loading'
	bl_idname = updater.addon + '.updater_loading'
	bl_description = f'Loading the latest version of {Extension}'
	bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_update(bpy.types.Operator):
	bl_label = 'Update'
	bl_idname = updater.addon + '.updater_update'
	bl_description = f'Update to the latest version of {Extension}'
	bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_latest(bpy.types.Operator):
	bl_label = f'Latest'
	bl_idname = updater.addon + '.updater_latest'
	bl_description = f'The latest version of {Extension} is installed'
	bl_options = {'REGISTER', 'INTERNAL'}
...
# class addon_updater_patch_notes(bpy.types.Operator):
# 	bl_label = 'Patch'
# 	bl_idname = updater.addon+'.updater_patch_notes'
# 	bl_description = f'Show the patch notes for the selected version of {Extension}'
# 	bl_options = {'REGISTER', 'INTERNAL'}

# 	@classmethod
# 	def poll(cls, context):
# 			if updater.invalidupdater or not version_list: return False
# 			return True
...
class addon_updater_update_target(bpy.types.Operator):
	...
	bl_description = f'Install a targeted version of {Extension}'
	bl_options = {'REGISTER', 'INTERNAL'}
...
class addon_updater_end_background(bpy.types.Operator):
	...
	bl_description = 'Stop searching for the latest update in the background'
	bl_options = {'REGISTER', 'INTERNAL'}
...
def update_notice_box_ui(self, context):
	...
	if updater.manual_only==False:
		colR.operator(addon_updater_update.bl_idname,
...
def update_settings_ui(self, context, element=None):
	...
	if updater.error != None and updater.error_msg != None:
		...
		col1.label(text=f'Error: {updater.error_msg}')
		...
		col1 = row.column(align=True)
		col1.label(text=f'Date: {lastcheck}')
		col2 = row.column(align=True)
		col2.alignment = 'RIGHT'
		col2.label(icon='TIME')
	else:
		col1 = row.column(align=True)
	...
	if updater.error != None:
		...
		if 'ssl' in updater.error_msg.lower():
			col.enabled = True
			col.operator(addon_updater_install_manually.bl_idname, text='Error') # updater.error
		else:
			col.enabled = False
			col.operator(addon_updater_search.bl_idname, text='Error') # updater.error

		col = row.column(align=True)
		col.scale_y = 1
		col.operator(addon_updater_refresh.bl_idname, text = '', icon='FILE_REFRESH')

	elif updater.update_ready == None and updater.async_checking == False:
		col = row.column(align=True)
		col.scale_y = 2
		col.operator(addon_updater_search.bl_idname, text='Search')
		...
		col.scale_y = 2
		col.enabled = False
		col.operator(addon_updater_loading.bl_idname, text='Loading...')

		col = row.column(align=True)
		col.scale_y = 1
		col.operator(addon_updater_end_background.bl_idname, text = '', icon='X')
		...
		col = row.column(align=True)
		col.scale_y = 2
		col.operator(addon_updater_update.bl_idname, text=f'Update {str(updater.include_branch_list[0])}')
		...
		col = row.column(align=True)
		col.scale_y = 2
		col.operator(addon_updater_update.bl_idname, text=f'Update {str(updater.update_version)}')
		...
		col.scale_y = 2
		col.enabled = False
		col.operator(addon_updater_latest.bl_idname, text='Latest')
	...
	if updater.manual_only == False:
		...
		# if updater.include_branches == True and len(updater.include_branch_list)>0:
		# 	row.operator(addon_updater_update_target.bl_idname, text='Version')
		# else:
		row.operator(addon_updater_update_target.bl_idname, text='Version')
		...
		# row.operator(addon_updater_search.bl_idname, text='Refresh')
		col.operator(addon_updater_restore_backup.bl_idname, text='', icon='RECOVER_LAST')
...
classes = (
	...
	addon_updater_search,
	addon_updater_refresh,
	addon_updater_loading,
	addon_updater_update,
	addon_updater_latest,
...
def register(bl_info):
	...
	updater.user = 'AceAsin'

	# choose your own repository, must match git name
	updater.repo = 'BlenderRPC'

	#updater.addon = # define at top of module, MUST be done first

	# Website for manual addon download, optional but recommended to set
	updater.website = 'https://github.com/AceAsin/BlenderRPC'
